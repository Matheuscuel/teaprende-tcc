...
async function canManageChild(user, childId) {
  ...
  if (user.role === ROLES.TERAPEUTA || user.role === ROLES.PROFESSOR) {
    const link = await prisma.child_professional.findFirst({ where: { child_id: child.id, professional_id: user.id }, select: { id:true } });
    return !!link;
  }
  return false;
}
...
// GET /api/games
router.get('/', authMiddleware, [ query('q').optional().isString() ], async (req, res) => {
...
router.post('/:id/progress',
  authMiddleware,
  [param('id').isInt({min:1}), body('child_id').isInt({min:1}), body('score').isInt({min:0,max:100}), body('time_spent').optional().isInt({min:0}), body('notes').optional().isString()],
  async (req, res) => {
    const game_id = Number(req.params.id);
    const { child_id, score, time_spent, notes } = req.body;

    if (!(await canManageChild(req.user, child_id))) return res.status(403).json({ message: 'Sem permissão' });
    ...
    const rec = await prisma.game_progress.create({
      data: { game_id, child_id: Number(child_id), score: Number(score), time_spent: time_spent ?? null, notes: notes ?? null }
    });
